...
if [ $? -eq 0 ]; then
	...
	if [ "$debug" = 'debug' ]; then
       ...
       -jar target/websocket-basic-java-client-1.0.jar --server.port=8085 1>>build.logs 2>>build.logs &
	else
		echo "starting server "
		java -Xmx2000M -Xms256M -jar target/websocket-basic-java-client-1.0.jar 1>>build.logs 2>>build.logs &
	fi
	echo $! >> process.pid
else
